...
public class MyThreadLocal<T> {
	 ...
	 */
	public MyThreadLocal(final T initialValue) {
		this(new Supplier<T>() {
			@Override
			public T get() {
				return initialValue;
	 ...
	 */
	public MyThreadLocal(final Supplier<T> initialValueCreator) {
		this.initialValueCreator = checkNotNull(initialValueCreator);
	}

	public static void printLocation(String msg, int depth, int num)
    {
    		if (Thread.currentThread().getName().equals("main"))
    			return;
    		StackTraceElement stackTrace =  Thread.currentThread().getStackTrace()[depth];
    		if (stackTrace.getClassName().contains("AuthorizationEnabledFilter"))
    			return;
    		
    		for (int i = depth; i < depth + num; i++)
    		{
    			String classname = Thread.currentThread().getStackTrace()[i].getClassName();
    			if (classname.contains("MetaDataStore") || classname.contains("TokenStore"))
    				return;
    		}
    		System.out.println(msg +"["+Thread.currentThread().getName()+"]:" );
    		for (int i = depth; i < depth + num; i++)
    		{
    			stackTrace =  Thread.currentThread().getStackTrace()[i];
	...
	public void set(final T value) {
		/*if ((int)value > 0)
		{
			System.out.println("Setting GraphID:"+value);
			printLocation("Set:", 2, 10);
		}*/
